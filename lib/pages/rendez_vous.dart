...
class _RendezVousState extends State<RendezVous> {
  ...
  Future<List<JourModel>> fetchJours() async {
    final response = await http.get(Uri.parse(
        'https://doctor-app-h45i.onrender.com/doctor/list_id_doctor/32/'));
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      final jours =
          parseJourModels(jsonData['jours'].cast<Map<String, dynamic>>());
      return jours;
    } else {
  ...
  List<JourModel> parseJourModels(List<dynamic> responseBody) {
    return responseBody
        .map<JourModel>((json) => JourModel.fromJson(json))
        .toList();
  }
